...
# First Correct Answer external-redundancy reward. Disable with
# first_correct_box_reward_enabled=False for an unshaped ablation.
...
# GRPO schedule.
epoch=${epoch:-10000}
project_name=${project_name:-DenoiseRL-v3-4B}
lr=${lr:-1e-6}
...
run_tag="rho${v2_initial_rho}-${v2_max_rho}_target${v2_target_accuracy}_alpha${v2_alpha}_window${v2_history_window}_slope${v2_slope_threshold}_${first_correct_box_reward_tag}"
experiment_name=${experiment_name:-"grpo-self-denoise-v3-${model_name}-bsz${train_prompt_bsz}-k16-${run_tag}"}
