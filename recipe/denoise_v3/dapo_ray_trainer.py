...
from recipe.denoise_v3.dynamic_rho import DynamicRhoController
from recipe.denoise_v3.first_correct_box_reward import (
    apply_first_correct_box_reward,
)
from recipe.denoise_v3.length_reward import (
    apply_dynamic_length_reward,
    apply_response_clip_penalty,
)
from recipe.denoise_v3.noise_metrics import compute_paired_noise_acc_metrics
from recipe.denoise_v3.per_sample_curriculum import (
    PerSampleNoiseCurriculum,
    has_usable_wrong_solution,
)
from recipe.denoise_v3.prefix_cut import cut_wrong_solution_prefix
from recipe.denoise_v3.random_token_noise import scaled_random_token_count
    ...
    * ``"none"``    — response width = ``R + max_partial_len``, NO truncation
    ...
    def _rollout_and_compute_reward(self, batch_dict, _metrics, timing_raw):
        ...
        with marked_timer("reward", timing_raw, "yellow"):
            ...
            first_correct_box_reward_enabled = bool(
                self.config.trainer.get(
                    "first_correct_box_reward_enabled", False
                )
            )
            _metrics["denoise/first_correct_box_reward/enabled"] = float(
                first_correct_box_reward_enabled
            )
            if first_correct_box_reward_enabled:
                correctness = reward_extra_infos_dict.get("acc")
                if correctness is None:
                    raise ValueError(
                        "trainer.first_correct_box_reward_enabled=True requires "
                        "the reward function to return one 'acc' value per rollout."
                    )
                compute_score = getattr(self.reward_fn, "compute_score", None)
                if not callable(compute_score):
                    raise ValueError(
                        "trainer.first_correct_box_reward_enabled=True requires "
                        "a reward manager exposing a callable compute_score."
                    )

                reward_fn_key = str(
                    getattr(self.reward_fn, "reward_fn_key", "data_source")
                )
                data_sources = new_batch.non_tensor_batch.get(reward_fn_key)
                reward_models = new_batch.non_tensor_batch.get("reward_model")
                if data_sources is None or reward_models is None:
                    raise ValueError(
                        "First-correct-box verification requires per-rollout "
                        f"{reward_fn_key!r} and 'reward_model' metadata."
                    )
                extra_infos = new_batch.non_tensor_batch.get("extra_info")
                if extra_infos is None:
                    extra_infos = np.asarray(
                        [{} for _ in range(len(new_batch))], dtype=object
                    )

                prompt_width = int(new_batch.batch["prompts"].shape[1])
                response_attention_mask = new_batch.batch["attention_mask"][
                    :, prompt_width:
                ]
                (
                    reward_tensor,
                    first_correct_box_extras,
                    first_correct_box_metrics,
                ) = apply_first_correct_box_reward(
                    reward_tensor,
                    responses=new_batch.batch["responses"],
                    response_attention_mask=response_attention_mask,
                    partial_response_lengths=partial_lens_np,
                ...
                _metrics.update(
                    {
                        f"denoise/first_correct_box_reward/{key}": value
                        for key, value in first_correct_box_metrics.items()
                    }
                )
