...
original reward. Correct rollouts keep factor ``1.0`` up to the final length
cache, then scale linearly to ``min_factor`` at ``max_response_length``.
"""
...
def linear_length_factor(
    ...
    *,
    ...
    length_cache: int = 1024,
    min_factor: float = 0.0,
) -> torch.Tensor:
    """Return factors that decrease only inside the final response-length cache."""
    if max_response_length <= 0:
        raise ValueError(
            f"max_response_length must be > 0, got {max_response_length}."
        )
    if not 0 < length_cache <= max_response_length:
        raise ValueError(
            "length_cache must satisfy 0 < length_cache <= max_response_length, "
            f"got {length_cache} and {max_response_length}."
        )
    ...
    penalty_start = max_response_length - length_cache
    cache_progress = (
        (lengths - float(penalty_start)) / float(length_cache)
...
def apply_correct_length_reward(
    ...
    *,
    ...
    Args:
        ...
        length_cache: Size of the final interval in which the factor decreases.
            Responses no longer than ``max_response_length - length_cache`` keep
            factor ``1.0``.
